...
export default function UploadZone({ onUploadSuccess, onCancel }) {
  ...
  const handleFile = async (file) => {
    ...
    if (!["pdf", "txt"].includes(ext))
      return toast.error("Only PDF and TXT files supported");
      ...
      >
        ...
        {uploading ? (
          <div style={styles.uploadingBox}>
            ...
            <p style={styles.uploadingSub}>
              First upload may take 2-3 mins while AI model loads
            </p>
            <p style={styles.uploadingSub}>
              Chunking → Embedding → Analyzing → Storing
            </p>
          </div>
        ) : (
          <>
